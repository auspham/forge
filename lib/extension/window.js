...
export class WindowManager extends GObject.Object {
  ...
  trackCurrentMonWs() {
    ...
    const currentMonWindows = global.display
      .get_tab_list(
        Meta.TabList.NORMAL_ALL,
        global.display.get_workspace_manager().get_active_workspace()
      )
      .filter((w) => w.get_monitor() === currentMonitor && w !== metaWindow);
    this.sortedWindows = global.display.sort_windows_by_stacking(currentMonWindows).reverse();
  }
  ...
  move(metaWindow, rect) {
    if (!metaWindow || (this.grabOp && metaWindow === this.focusMetaWindow)) return;
    try {
      // GNOME 49+
        ...
        if (!attachTarget) {
          ...
          if (hasWindows) {
            if (attachTarget && metaMonWsNode.contains(attachTarget)) {
              // If the attach target is inside a tabbed/stacked container,
              // place new windows alongside the group instead of inside it
              let attachParent = attachTarget.parentNode;
              if (attachParent) {
                let parentLayout = attachParent.layout;
                if (parentLayout === LAYOUT_TYPES.TABBED || parentLayout === LAYOUT_TYPES.STACKED) {
                  attachTarget = attachParent;
                }
              }
            } else {
  ...
  updateMetaWorkspaceMonitor(from, _monitor, metaWindow) {
    if (this._validWindow(metaWindow)) {
      ...
      // Skip reparenting for windows that haven't been rendered by the tree yet.
      // The tree's initial placement from trackWindow should take precedence over
      // the physical monitor reported by window-entered-monitor signals during creation.
      if (metaWindow.firstRender) return;
      // Skip reparenting while the tree is actively applying layout positions.
      // Signals fired during apply() can report stale/transient monitor assignments
      // which would corrupt the tree and cause infinite monitor-jumping oscillation.
  ...
  moveWindowToPointer(focusNodeWindow, preview = false) {
    ...
    if (nodeWinAtPointer) {
      ...
      const leftRegion = hoverRegions.left;
      const rightRegion = hoverRegions.right;
      const topRegion = hoverRegions.top;
      const bottomRegion = hoverRegions.bottom;
      const centerRegion = hoverRegions.center;
      ...
      if (isCenter) {
        ...
        if (stackedOrTabbed) {
          ...
          if (!isMonParent) {
            ...
            // Parent is MONITOR with stacked/tabbed layout - detach alongside
            containerNode = parentNodeTarget;
            referenceNode = nodeWinAtPointer;
      ...
      if (!preview) {
        ...
        if (childNode.createCon) {
          ...
          if (
            ...
            childNode.settings = this.ext.settings;
