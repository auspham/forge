...
export class Node extends GObject.Object {
  ...
  _createWindowTab() {
    ...
    let closeFn = () => {
      metaWin.delete(global.display.get_current_time());
    };
  ...
  render() {
    ...
    if (this.tab !== null && this.tab !== undefined) {
      ...
      if (titleLabel && titleLabel.set_label) {
  ...
  resetLayoutSingleChild() {
    ...
    if (tabbedOrStacked && this.singleOrNoChild()) {
      // Use monitor geometry to determine correct split direction
      // instead of always defaulting to HSPLIT
      let monitorRect = global.display.get_monitor_geometry(global.display.get_current_monitor());
      this.layout =
        monitorRect && monitorRect.width < monitorRect.height
          ? LAYOUT_TYPES.VSPLIT
          : LAYOUT_TYPES.HSPLIT;
    }
  }
...
export class Tree extends Node {
  ...
  swapPairs(fromNode, toNode, focus = true) {
    ...
    if (parentForTo && parentForFrom) {
      ...
      if (focus) {
        ...
        fromNode.nodeValue.focus(global.display.get_current_time());
      }
    }
  }
  ...
  apply(node) {
    ...
    let tiledChildren = node
      .getNodeByMode(Window.WINDOW_MODES.TILE)
      .filter((t) => t.nodeType === NODE_TYPES.WINDOW);
    this.extWm._applyingLayout = true;
    ...
    this.extWm._applyingLayout = false;
  }
  ...
  processTabbed(node, child, params, _index) {
    ...
    if (layout === LAYOUT_TYPES.TABBED) {
      ...
      if (node.childNodes.length > 1 || alwaysShowDecorationTab) {
        ...
        if (node.decoration && child.isWindow() && child.actor && child.actor.border) {
          let gap = this.extWm.calculateGaps(node);
          let renderRect = this.processGap(node);
          let themeNode = child.actor.border.get_theme_node();
          let borderWidth = themeNode ? themeNode.get_border_width(St.Side.TOP) : 0;
          ...
          if (decoration !== null && decoration !== undefined) {
            ...
            if (child.tab && !decoration.contains(child.tab)) decoration.add_child(child.tab);
          }

          if (child.tab) child.render();
        }
      }
